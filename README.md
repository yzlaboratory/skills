...
6. **Implement** — pick one:
   ...
   - **`/implement-issues`** to fan the whole backlog out to parallel subagents, each implementing one issue in its own git worktree. It asks once how subagents should implement — TDD, test-after, direct, `/kira-style-implementation`, or a custom prompt — then waves spawn automatically as blockers clear, with no per-wave gate.
...
- [`/implement-issues`](./skills/engineering/implement-issues/SKILL.md) — orchestrates parallel subagents, one per issue per wave, each in its own worktree. Asks once how subagents should implement, then waves spawn automatically as blockers clear, with no per-wave gate.
