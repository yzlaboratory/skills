...
- **A new spec is needed for a behavior that does not yet have one.** Every spec — newly-authored or rewritten — **must follow `docs/specs/_template.md` exactly**, with `docs/specs/README.md` providing the surrounding convention (Gherkin discipline, prose-vs-fenced-block placement, ADR cross-linking). If either file is missing, scaffold them from this skill's bundled defaults at [`spec-defaults/_template.md`](./spec-defaults/_template.md) and [`spec-defaults/README.md`](./spec-defaults/README.md) — copy them into the project's `docs/specs/` directory verbatim, tell the user what was scaffolded so they can customise, and then proceed with spec authoring. The same template adherence applies when editing an existing spec: keep the file's structure aligned with `docs/specs/_template.md`.
