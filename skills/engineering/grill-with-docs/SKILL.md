...
- **A new spec is needed for a behavior that does not yet have one.** Every spec — newly-authored or rewritten — **must follow `docs/specs/_template.md` exactly**, with `docs/specs/README.md` providing the surrounding convention (Gherkin discipline, prose-vs-fenced-block placement, ADR cross-linking). If `docs/specs/_template.md` is missing, do not invent a structure — surface the gap to the user, keep notes of the spec rewrites the session has accumulated, and pause spec authoring until the template is in place. The same applies when editing an existing spec: keep the file's structure aligned with the template.
