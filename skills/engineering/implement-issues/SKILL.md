...
description: Orchestrate parallel implementation of every implementation issue under a feature ticket, using subagents that each implement one issue in an isolated git worktree. Asks up front how subagents should implement — TDD, test-after, direct implementation, the kira-style-implementation skill, or a custom prompt. Use when user wants to bulk-implement a backlog generated by /to-issues, run many tickets concurrently, asks for parallel issue implementation, or runs /implement-issues. Takes the feature ticket (issue number / Story key), or derives it from the current branch.
---
...
- **kira-style-implementation** — each subagent runs the `/kira-style-implementation` skill.
