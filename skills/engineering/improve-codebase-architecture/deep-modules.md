...
The full vocabulary — depth, leverage, locality, seam, adapter — lives in [`LANGUAGE.md`](LANGUAGE.md). Use those terms when discussing architecture; this file zooms in on depth.

## Why depth matters

The interface is also the test surface. A deep interface gives you a small test surface that exercises a lot of behaviour — the same leverage callers get, your tests get too. A shallow interface forces thin tests against a thin layer, and the real bugs hide in how the layers compose.

When designing the interface for a behaviour, ask:
