...
Default: **no comment.** Write one only when it is *absolutely necessary* to understand a why — and a why needs an inline comment only when it can be moved neither *into the code itself* (a name, a type, a structure, a test) **nor out to a durable doc that already records it** (an ADR). If a name can carry it, rename; if a type can enforce it, encode it; **if an ADR documents the decision, cut the comment — the reader finds the why there.** The bar is high on purpose: most comments that feel justified are restating a decision that already has a home. Before writing one, in order:
...
- **The ADR-documented why.** If a why is already recorded in an ADR (or another durable doc), the ADR is its home — do **not** restate it inline, not even as a one-clause anchor or a bare `// per ADR 0024` pointer. This is the trap that survives a careless review: a header reads as "keep" because it *contains* a why — but a why with a documented home is not one the code must carry, it is one the reader finds in `docs/adr/`. Cut the whole comment; the class name plus the ADR suffice. Examples that go: a controller header explaining that signup answers identically so the response can't be used to enumerate accounts (ADR 0024); an exception's header explaining how the web layer maps it to a generic response (ADR 0024); a dispatcher header explaining why mail goes off-thread to close a timing oracle (ADR 0024). What stays inline is only the strictly-*local* implementation why no ADR carries — why this ordering, why this catch arm, the StrictMode dedupe. If the ADR link is worth recording at all, it goes in the commit message, not the source.
...
This is why a `move` verdict carries a destination: record the why in its proper home, *then* cut the inline line. A why that has a durable home (an ADR) keeps **no** anchor clause in the source — it is cut whole, not trimmed to a pointer. The "provenance → commit message" and "ADR-documented why → cut" rules in the delete-on-sight list above are this same routing in its negative form: the prohibition is on restating, inline, a why that lives elsewhere — never on the why existing. What is left inline is only the strictly-local why no other home will hold.
